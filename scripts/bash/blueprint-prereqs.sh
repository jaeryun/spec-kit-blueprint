...
#   "SPEC_OUTLINES": [
#     {"id": "SO-01", "goal": "Users can register...", "spec_linked": "docs/spec/auth.md"},
#     {"id": "SO-02", "goal": "...", "spec_linked": ""}
#   ]
# }
#
...
_emit_so_entry() {
    ...
    local escaped_id escaped_goal escaped_spec
    ...
    _so_outlines="${_so_outlines}{\"id\":\"${escaped_id}\",\"goal\":\"${escaped_goal}\",\"spec_linked\":\"${escaped_spec}\"}"
}
...
if $JSON_MODE; then
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        # jq 없을 때 heredoc fallback — paths escaped to prevent JSON breakage
